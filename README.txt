Author: Jainul N Vaghasia (jainulvaghasia@gmail.com)

Write up for Columbia Phd Admissions application
...
Major changes made in order to replace LRU were to change the do_item_alloc(), item_link_q(), do_item_get(), and do_item_update() methods.
...
2.
It should be fairly noticable that the LRU policy is not scalable for the fact that it acquires global cache_locks in order to rearrange
the list. We can instead use a granular locking policy with a log for LRU rearrangements when performing reading operations like GET
(STORE, DELETE can still use a global lock, nobody likes them anyways). Rearrangements can then eventually happen using the log.
The key idea here is to reduce critical section of global locks as much as possible.

The pipeline would look like

    GET(x) -----> Return(Value) ---> Record Log ----> Perform in batches
...
Perhaps the similar methods mentioned in part 1 will have to be changed to allow for the log. When we do_item_update(), we need to ensure
